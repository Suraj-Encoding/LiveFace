...
if __name__ == "__main__":
    ...
    if args.input:  # file
        ...
        # On macOS, don't use CAP_DSHOW which is Windows-specific
        vid_capture = cv2.VideoCapture(0)
    
    # Check if video stream opened successfully
    if not vid_capture.isOpened():
        print("Error opening a video stream")
        exit(1)  # Exit the program if video stream cannot be opened
        
    # Get frame dimensions after confirming video is open
    frame_width = int(vid_capture.get(3))
    frame_height = int(vid_capture.get(4))
    frame_size = (frame_width, frame_height)
    print('Frame size  :', frame_size)

    # Initialize default fps
    fps = 24
    
    # Reading fps and frame rate
    fps_from_video = vid_capture.get(5)  # Get information about frame rate
    if fps_from_video > 0:
        fps = fps_from_video
